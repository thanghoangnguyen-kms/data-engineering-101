---
tags:
  - DE101
  - roadmap
date: 2026-06-28
status: in-progress
---

# 🗺️ Engineering 101 — Onboarding Roadmap

> [!NOTE] About This Vault
> This vault covers two parallel learning tracks for intern engineers.
> Choose the track that matches your role — or complete both.
>
> **New here?** Read [[00.1 - How to Use This Vault]] first.

---

## 🛣️ Choose Your Track

| Track | Description | Start Here |
|-------|-------------|-----------|
| 🖥️ **Backend Engineering** | Python, FastAPI, PostgreSQL, testing, Docker, microservices | [[Backend/00 - Backend Track Roadmap\|Backend Track Roadmap]] |
| 🗄️ **Data Engineering** | SQL, data modeling, storage formats, batch pipelines, cloud | [[DataEngineering/00 - DE Track Roadmap\|DE Track Roadmap]] |

> [!TIP] Not sure which to pick?
> - Joining as a **backend/software engineer** → start with the Backend track
> - Joining as a **data engineer** → start with the DE track
> - The tracks share foundations (Python, Git, Linux) — if you've done the Backend track, you can skim D1

---

## 🗂️ Track Summaries

### 🖥️ Backend Track

| #   | Domain                                                                                 | Status          | Checkpoint                                                  |
| --- | -------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------- |
| 1   | [[Backend/B1 - Foundations & Dev Setup\|B1 — Foundations & Dev Setup]]                 | ✅ Complete      | [[Backend/Checkpoints/CB1 - Dev Environment Ready\|CB1]]    |
| 2   | [[Backend/B2 - Web & API Fundamentals\|B2 — Web & API Fundamentals]]                   | ✅ Complete      | [[Backend/Checkpoints/CB2 - API Built & Documented\|CB2]]   |
| 3   | [[Backend/B3 - Databases & ORM\|B3 — Databases & ORM]]                                 | ✅ Complete      | [[Backend/Checkpoints/CB3 - DB & ORM Proficiency\|CB3]]     |
| 4   | [[Backend/B4 - Authentication & Security\|B4 — Authentication & Security]]             | ✅ Complete      | [[Backend/Checkpoints/CB4 - Auth & Security Verified\|CB4]] |
| 5   | [[Backend/B5 - Testing & Code Quality\|B5 — Testing & Code Quality]]                   | ✅ Complete      | [[Backend/Checkpoints/CB5 - Test Suite Complete\|CB5]]      |
| 6   | [[Backend/B6 - Async, Queues & Background Jobs\|B6 — Async, Queues & Background Jobs]] | ✅ Complete      | [[Backend/Checkpoints/CB6 - Queue & Workers Running\|CB6]]  |
| 7   | [[Backend/B7 - Microservices & Containers\|B7 — Microservices & Containers]]           | ✅ Complete     | [[Backend/Checkpoints/CB7 - Service Containerised\|CB7]]    |
| 8   | [[Backend/B8 - Capstone Project\|B8 — Capstone Project]]                               | ✅ Complete     | [[Backend/Checkpoints/CB8 - Capstone Complete\|CB8]]        |

### 🗄️ Data Engineering Track

| # | Domain | Status | Checkpoint |
|---|--------|--------|-----------|
| 1 | [[DataEngineering/D1 - Foundations & Tooling\|D1 — Foundations & Tooling]] | ✅ Complete | [[DataEngineering/Checkpoints/CP1 - Tooling & Environment Ready\|CP1]] |
| 2 | [[DataEngineering/D2 - SQL & Data Modeling\|D2 — SQL & Data Modeling]] | ✅ Complete | [[DataEngineering/Checkpoints/CP2 - SQL Proficiency\|CP2]] |
| 3 | [[DataEngineering/D3 - Data Storage & Formats\|D3 — Data Storage & Formats]] | ✅ Complete | [[DataEngineering/Checkpoints/CP3 - Storage & Modeling\|CP3]] |
| 4 | [[DataEngineering/D4 - Batch Processing & ETL\|D4 — Batch Processing & ETL]] | 🔴 Not started | [[DataEngineering/Checkpoints/CP4 - Batch Pipeline\|CP4]] |
| 5 | [[DataEngineering/D5 - Stream Processing\|D5 — Stream Processing]] | ✅ Complete | [[DataEngineering/Checkpoints/CP5 - Stream Pipeline\|CP5]] |
| 6 | [[DataEngineering/D6 - Cloud & Orchestration\|D6 — Cloud & Orchestration]] | 🔴 Not started | [[DataEngineering/Checkpoints/CP6 - Cloud Deployment\|CP6]] |
| 7 *(optional)* | [[DataEngineering/D7 - AI-Ready Data Engineering\|D7 — AI-Ready Data Engineering]] | 🔵 Optional | [[DataEngineering/Checkpoints/CP7 - AI Data Engineering\|CP7]] |
